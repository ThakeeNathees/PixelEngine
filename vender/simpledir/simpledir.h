...
inline std::vector<std::string>& str_split(std::vector<std::string>& vector, std::string str, std::string delimiter = "/")
{
	vector.clear();
		size_t pos = 0;
		std::string token;
...
class SimpleDir
	...
	public:
		inline SimpleDir() : m_path("./"), dir(NULL), ent(NULL) {}
		inline ~SimpleDir(){delete dir;}
		
		/* chack if path is a directory */
		inline static bool isDirectory(const std::string& path) {
			struct stat buffer;
		...
		inline static int mkDir(const std::string& path, bool make_sub_dir = true) {
		...
		inline static int rmDir(const std::string& path, bool del_sub_dir = true){
			int error;
			if (!del_sub_dir) {
				error = _rmdir(path.c_str());
				if (error && SimpleDir::isDirectory(path))
					return FILE_NOT_EMPTY;
			}
			else
			{
				SimpleDir file; file.open(path);
				for (auto f : file.getFiles()) {
					if (!SimpleDir::isDirectory(f))
						error = remove(f.c_str());
					else
						error = SimpleDir::rmDir(f, true);
				}
				error = _rmdir(path.c_str());
			}
			if (!error) return FILE_SUCCESS;
